...
class CheckupHouseStepBuildingOutdoor extends StatefulWidget {
  final StepData step;
  final CheckupHouseCaseObject caseObject;
  const CheckupHouseStepBuildingOutdoor(this.step, this.caseObject, {super.key});

  @override
  State<CheckupHouseStepBuildingOutdoor> createState() => _CheckupHouseStepBuildingOutdoorState();
}

class _CheckupHouseStepBuildingOutdoorState extends State<CheckupHouseStepBuildingOutdoor> {
    ...
    final stepContent = CheckupHouseStepContent(
      ...
      content: Column(
        ...
        children: [
          ...
          const Text(
            'Сфотографируйте строение со всех четырёх сторон или с противоположных углов (минимум с двух углов). При этом строение должно попадать в кадр целиком (от фундамента до кровли и от его левого угла до правого). При необходимости делается несколько фотографий с разных расстояний.',
            style: TextStyle(fontSize: 16, height: 1.25),
          ),
          const SizedBox(height: 15),
          ClipRRect(
            borderRadius: BorderRadius.circular(borderRadiusBig),
            child: Image.asset('assets/images/checkup_house/step_outdoor_01.jpg'),
          ),
          const SizedBox(height: 15),
