...
class CheckupHouseStepBuildingIndoors extends StatefulWidget {
  final StepData step;
  final CheckupHouseCaseObject caseObject;
  const CheckupHouseStepBuildingIndoors(this.step, this.caseObject, {super.key});

  @override
  State<CheckupHouseStepBuildingIndoors> createState() => _CheckupHouseStepBuildingIndoorsState();
}

class _CheckupHouseStepBuildingIndoorsState extends State<CheckupHouseStepBuildingIndoors> {
    ...
    final stepContent = CheckupHouseStepContent(
      ...
      content: Column(
        ...
        children: [
          ...
          const Text(
            'Фото всех внутренних помещений дома. Минимум четыре фото на одно помещение',
            style: TextStyle(fontSize: 16, height: 1.25),
          ),
          const SizedBox(height: 15),
          RichText(
            text: const TextSpan(
              text: 'Сначала делаются фото на первом этаже, затем ',
              style: TextStyle(fontSize: 16, height: 1.25, color: Colors.black),
              children: [
                TextSpan(
                  text: 'фото лестницы в доме',
                  style: TextStyle(
                ...
                TextSpan(
                  text: ' и далее фото помещений на вышерасположенных этажах, при наличии.',
                ),
              ],
            ),
          ),
          const SizedBox(height: 15),
          const Text(
            'Внутри каждого помещения должны быть сфотографированы примеры типовой для данного строения отделки пола, стен, потолков, окон, межкомнатных дверей.',
            style: TextStyle(fontSize: 16, height: 1.25),
          ),
          const SizedBox(height: 15),
          ClipRRect(
            borderRadius: BorderRadius.circular(borderRadiusBig),
            child: Image.asset('assets/images/checkup_house/step_indoors_01.jpg'),
          ),
          const SizedBox(height: 15),
