...
class CheckupHouseStepOutside extends StatefulWidget {
  final StepData step;
  final CheckupHouseCaseObject caseObject;
  const CheckupHouseStepOutside(this.step, this.caseObject, {super.key});

  @override
  State<CheckupHouseStepOutside> createState() => _CheckupHouseStepOutsideState();
}

class _CheckupHouseStepOutsideState extends State<CheckupHouseStepOutside> {

  final bool isBuildingStep = false;
  final GlobalKey<CameraPhotosContainerState> _imagesContainer = GlobalKey();

  // сюда доставляется фотка от фотоаппарата
  ...
  @override
  void initState() {
    updateStepStatus(widget.step, widget.caseObject);
    super.initState();
  }
    ...
    final stepContent = CheckupHouseStepContent(
      ...
      content: Column(
        ...
        children: [
          ...
          const Text(
            'Сфотографируйте все строения, подлежащие страхованию, со стороны подъездных путей и ограждений, с привязкой к адресу (при наличии адресной таблички).',
            style: TextStyle(fontSize: 16, height: 1.25),
          ),
          const SizedBox(height: 15),
          ClipRRect(
            borderRadius: BorderRadius.circular(borderRadiusBig),
            child: Image.asset('assets/images/checkup_house/step_outside_01.jpg'),
          ),
          const SizedBox(height: 15),
