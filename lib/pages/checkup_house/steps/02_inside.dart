...
class CheckupHouseStepInside extends StatefulWidget {
  final StepData step;
  final CheckupHouseCaseObject caseObject;
  const CheckupHouseStepInside(this.step, this.caseObject, {super.key});

  @override
  State<CheckupHouseStepInside> createState() => _CheckupHouseStepInsideState();
}

class _CheckupHouseStepInsideState extends State<CheckupHouseStepInside> {
    ...
    final stepContent = CheckupHouseStepContent(
      ...
      content: Column(
        ...
        children: [
          ...
          const Text(
            'Сделайте фото участка внутри, с противоположных по диагонали углов (на фотографиях должны быть запечатлены окружающие строения объекты (строения на соседних участках, деревья и т.п.)',
            style: TextStyle(fontSize: 16, height: 1.25),
          ),
          const SizedBox(height: 15),
          ClipRRect(
            borderRadius: BorderRadius.circular(borderRadiusBig),
            child: Image.asset('assets/images/checkup_house/step_inside_01.jpg'),
          ),
          const SizedBox(height: 15),
